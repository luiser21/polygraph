...
class sql{
     ...
     * @return
     */
    //function sql($host = 'localhost', $user = 'dnaonlin_cursos', $pass = 'Q![;iI0tAX-_', $db = 'dnaonlin_cursos') 
    function sql($host = 'localhost', $user = 'root', $pass = '', $db = 'cursos_dna')
    {
      /*  if(!$this->p){
       ...
       // return $this->mysqli;
        //print_r($this->mysqli); die;
       // $this->IdConexion = $connect;
//        if ($connect) {
//            $db_connect = @mysql_select_db($connect,$db);
//            if ($db_connect) {
//            } else {
//                echo "<font color='red'>Connection whit database is down.</font>";
//            }
//            $this->IdConexion = $connect;
//            return $this->IdConexion;
//        } else {
//            echo "<font color='red'>Connection whit server is down </font>";
//        }
    }    
...
/*	public function ControlError($Error, $Tabla = '')
	{
			...
			{$this->IdConexion
				case "1062":
					...
					$ShowError .= " <br> Se encontraron uno o m&aacute;s usuarios repetidos por favor verifique el archivo. <B><br> ".$Tabla. mysql_error(). "</B> <br>";
	...
	*/
	 ...
	 * @return
	 */
	public function Consulta($Consulta,$showQuery = ""){
        $result = $this->mysqli->query($Consulta); 
        $counter = 0;
        $r = array();
        if($this->mysqli->errno)
            echo 'Error numero: '. $this->mysqli->errno.' <hr> Verificar:'.$this->mysqli->error.'<hr> Consulta:'. $Consulta. ' <hr>';
