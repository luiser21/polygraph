...
class Index extends crearHtml{
    ...
    public $_datos = '';
    public $Table = 'archivos_app';
    public $PrimaryKey = 'id_archivos_app';
    ...
    function Iniciar(){
         ...
         if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) || isset($_REQUEST['uploadFileIframe'])){
            ...
            $this->_datosm =  ( isset($_REQUEST['m']) && ( is_array($_REQUEST['m']) || is_object($_REQUEST['m']) ) )? $this->serializeArray(@$_REQUEST['m']) : @$_REQUEST['m'];
            ...
            <div class="col-md-8">
                        <div class="panel panel-default panelCrear">
                            ...
                            <div class="panel-body">
                                <form id="formArchivos" role="form">
                ...
                <div class="row panelArchivo" style="display:none">
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body" id="listaArchivos">
                                LISTA ARCHIVOS
                            </div>
                        </div>
                    </div>
    ...
    function runEliminar(){
         ...
         $sql = 'SELECT t.`descripcion` FROM '.$this->Table.' t
          WHERE t.id_archivos_app = '.$this->_datos;
          $datos = $this->Consulta($sql);
        @unlink('./archivosApp/'.$datos[0]['descripcion']);
        
        $sqlDel = "DELETE FROM ".$this->Table ." WHERE ".$this->PrimaryKey." = ".$this->_datos;
        $this->QuerySql($sqlDel);
    ...
    function tablaDatos(){
        ...
        $idCapitulo = ( isset($this->_datos['id_capitulo']) ) ? $this->_datos['id_capitulo'] : ''; 
        if($this->_datos=='r'){
            $idCapitulo = $this->_datosm['id_capitulo'];
        }
        
        
        $editar = $this->Imagenes($this->PrimaryKey,0);
        $eliminar = $this->Imagenes($this->PrimaryKey,1,'formArchivos');
        $sql = 'SELECT t.`descripcion`,'.$eliminar.' FROM '.$this->Table.' t
        
          WHERE t.id_capitulo = '.$idCapitulo;
