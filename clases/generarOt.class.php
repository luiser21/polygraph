                        ...
                        <div class="panel panel-default panelCrear">
                            ...
                            <div class="panel-body">
                                <form id="formCrear" role="form">
                                    ...
                                    <div class="form-group">
                                        <label for="fecha_fin">Tipo Frecuencia:</label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        '.$this->create_input('date','fecha_ini','fecha_ini','AAAA-MM-DD',false,'form-control required').'
                                        
                                    </div>
                                    
                                    <div class="form-group">
                                        <label for="id_cursos">Fecha Fin:</label>
                                        '.$this->create_input('date','fecha_fin','fecha_fin','AAAA-MM-DD',false,'form-control required').'
...
VALUES
  ...
  public function guardaDetalleOt(){  
    ...
    `minutos_ejec_real`
        ...
        FROM
            ...
            INNER JOIN metodos met ON met.id_metodos = mec.id_metodos
            INNER JOIN tareas tar ON tar.id_tareas = mec.id_tareas
            INNER JOIN lubricantes lub ON lub.id_lubricantes = mec.cod_lubricante
            INNER JOIN unidades uni ON uni.id_unidades = mec.id_unidad_cant
            INNER JOIN aplicaciones_lub apl ON apl.id_aplicacion = mec.id_aplicacion
    ...
    $sqlUpdate = "UPDATE ot_detalle 
                    SET `fecha_prog` = '".$this->_datos['fecha_ini']."', 
                        `fecha_real` = '".$this->_datos['fecha_ini']."' 
                    WHERE fecha_real < '".$this->_datos['fecha_ini']."' 
                    AND id_ot = $idOt";
    $this->QuerySql($sqlUpdate);
  ...
  if($this->_datos['id_unidad'] == "Horas"){
    ...
    $sql = "SELECT
            ...
            INNER JOIN lubricantes lub ON lub.id_lubricantes = mec.cod_lubricante
            INNER JOIN unidades uni ON uni.id_unidades = mec.id_unidad_cant
            INNER JOIN frecuencias fre ON fre.id_frecuencias = mec.id_frecuencias
            INNER JOIN aplicaciones_lub apl ON apl.id_aplicacion = mec.id_aplicacion
            AND pl.id_planta = '".$this->_datos['id_planta']."' 
	    WHERE meca.id_mecanismos not in (select d.id_mecanismos from ot_detalle d inner join ot o on (d.id_ot=o.id_ot) where o.estado=0)
        ".$whereHoras;
