...
class Index extends crearHtml{
    ...
    function ValidarUsuario($me){	
		...
		if($datosUsuario[0]['id_facebook']==''){	
			...
			if($datosUsuario[0]['foto']==0){				
				$_SESSION["foto"]		=   'archivosApp/usuario.jpg';
			}
			if($datosUsuario[0]['foto']==1){				
				$_SESSION["foto"]		=   'archivosApp/'.$_SESSION["id_usuario"].'.jpg';
			}

			if($datosUsuario[0]['foto']==2){				
				$_SESSION["foto"]		=   'http://graph.facebook.com/'.$datosUsuario[0]['id_facebook'].'/picture?type=large';
			}			
...
<table width="100%" border="0">
  ...
  <tr>
       ...
       if(!$this->_datos['id_usuario']){
		   if(count($this->consultarRegitro($this->_datos['numero_doc']))==0){
				$sql = "INSERT INTO ".$this->Table."(email,nombres,apellidos,tipo_documento,numero_documento,telefono,pass,fecha_registro) 
						VALUES 
						('".$this->_datos['email']."','".$this->_datos['nombre']."','".$this->_datos['apellido']."','".$this->_datos['tipo_documento']."','".$this->_datos['numero_doc']."','".$this->_datos['telefono']."','".md5($this->_datos['input7'])."','".date('Y-m-d')."')";
				
					$this->QuerySql($sql);
					$this->enviarMail($this->_datos['email'],$this->_datos['input7']);
					$mensaje = true;
		   }else{
			   $mensaje = false;
			}
       }else{
           $sql = 'UPDATE '.$this->Table ." SET email = '".$this->_datos['email']."', nombres = '".$this->_datos['nombre']."', apellidos = '".$this->_datos['apellido']."', tipo_documento = '".$this->_datos['tipo_documento']."', numero_documento = '".$this->_datos['numero_doc']."', telefono = '".$this->_datos['telefono']."', pass = '".md5($this->_datos['input7'])."', = '".date('Y-m-d')."' WHERE ".$this->PrimaryKey." = ".$this->_datos['id_usuario'];
            $this->QuerySql($sql); 
       }
    ...
    function getDatos($me){
        $sql = 'SELECT '.$this->PrimaryKey .',email,nombres,apellidos,tipo_documento,numero_documento,telefono,pass,fecha_registro,id_facebook,foto 
