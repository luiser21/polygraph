...
class Index extends crearHtml{
    ...
    */
	...
	function reseteaContrasena($codigoReseteo){
	   $sql = "SELECT fecha_recordar,`id_usuario`,`email` FROM usuarios WHERE codigo_reseteo = '$codigoReseteo'";
       $datos = $this->Consulta( $sql ) ;
       if( count( $datos ) ){
            ...
            echo 'No cumple con la validación de datos, no se a realizado ninguna operación';
       }
	}
     ...
     * @author Javier Ardila javier.ardila.ext@claro.com.co
     */
    ...
    function actualizaContrasena($idUsuario){
        if( $this->aleatorio ){
            $sql = "UPDATE usuarios SET codigo_reseteo = '', `pass` = MD5('{$this->aleatorio}') WHERE `id_usuario` = {$idUsuario} ";
            $this->QuerySql($sql);
        }else{
...
<table width="100%" border="0" align="center" style=" font-family:Arial, Helvetica, sans-serif; color:#333; font-size:12px;">
  ...
  <tr>
    <td width="542" align="center"><a href="#"><img name="" src="../assets/img/Logo.png" width="290" height="57" alt="" /></a></td>
  </tr>
  <tr>
    <td align="center">&nbsp;</td>
  </tr>
  ...
  <tr>
    ...
    function actualizaCodigo(){
        $sql = "UPDATE usuarios SET codigo_reseteo = '{$this->aleatorio}', `fecha_recordar` = NOW() WHERE `email` = '{$this->correo}' ";
        $this->QuerySql($sql);
    }
    ...
    public function verificaCorreo(){
        $sql = 'SELECT id_usuario,pass FROM usuarios WHERE email = "'.$this->correo.'" ';			
			$datos = $this->Consulta($sql);	
            if( count( $datos ) ){
                return true;
            }else{
	...
	function  confirmaReseteoContrasena(){
		$to 	= $this->correo;
		$urlRecuperar = 'www.dnamusiconline.com/cursos_online/recuperar.php?cod=' . $this->aleatorio;
		$this->message = '
<body>
<table width="100%" border="0" align="center" style=" font-family:Arial, Helvetica, sans-serif; color:#333; font-size:12px;">
  <tr>
  ...
  <tr>
	...
	mail($to,'DNA MUSIC',$this->message,$headers);
	echo '
		Te enviamos la informacion a tu correo.	
		';
}
 ...
 <section class="container animated fadeInUp">
                    ...
                    <header><meta http-equiv="Content-Type" content="text/html; charset=windows-1252">
                        <div class="brand">
                            <a href="index.html" class="logo">
                                <img src="./assets/img/logo.png" width="163" height="32" />    
                            </a>								
                        </div>
                        <div class="brand">                         
                                ...
                                <div class="form-group">
                                    <div class="col-md-12" align="center">
                                        <a href="#"  id="recordar" class="help-block">Olvidaste Tu Contrase&ntilde;a?</a>
                                         <button type="button" id="guardaringreso" class="btn btn-primary btn-block">Ingresar</button>
                                    </div>
    ...
    */
    function login(){ 
        ...
        $_SESSION["bdatos"] = 'cartas_'.$httpHost[0];
        $email=$_POST['email'];
		...
		$sql = 'SELECT u.'.$this->PrimaryKey.', u.tipo_usuario, u.activo, u.nombres, u.apellidos, u.ciudad, u.foto, u.id_facebook,
                u.email,u.duenocliente,p.ruta,up.id_parametro FROM '.$_SESSION["bdatos"].'.'.$this->Table.' u
                INNER JOIN '.$_SESSION["bdatos"].'.usuarios_parametros up ON u.id_usuario = up.id_usuario
                INNER JOIN '.$_SESSION["bdatos"].'.parametros p ON p.id_parametro = up.id_parametro
		...
		if(count($datos)>0){  
			...
			/*if(isset($empresa) && $empresa<>''){
    			 $sql2 = "SELECT p.ruta,up.id_parametro
                        FROM ".$_SESSION["bdatos"].".usuarios u
                        INNER JOIN ".$_SESSION["bdatos"].".usuarios_parametros up ON u.id_usuario = up.id_usuario
                        INNER JOIN ".$_SESSION["bdatos"].".parametros p ON p.id_parametro = up.id_parametro
                         WHERE u.email ='".$email."' and p.id_parametro=$empresa";
    			$datos2 = $this->Consulta($sql2);
    			$var=true;
    			//var_dump($datos2[0]['ruta']);
    			//exit;
			}*/
			//exit;
            ...
            $_SESSION["empresa"]        = $datos[0]['id_parametro'];
