...
function fn_guarda(){
    ...
    if(validar_all('required')){
        ...
        $.post(file,{ 'process':'guardarDatos', 'd':datos},function(r){
            ...
            if( $('#guardarCurso').val() != 'Editar Registro' ){
                $('#formCrear')[0].reset(); 
            }
            var table = $('#tablaDatos').DataTable();
            // #myInput is a <input type="text"> element
            var Busqueda = $('.dataTables_filter input').val();
            recargaDatos('formCrear',Busqueda);
        },'json');
...
function fn_guardarMecMet(){
    var datos = $('#formCrear').serializeArray();
    //$("#divMecMet").fadeIn("slow");
     

    
    $.post(file,{ 'process':'guardarMecanismos', 'd':datos},function(r){
        $("#Resultado").fadeOut("slow");
        $("#Resultado").fadeIn(3000).html(r);
        $('.id_mecanismos').val(r.id);
        
        $('#mecTitulo').html(r.nombreMecaniso);
...
function fn_guardarMecMetModal(){
    ...
    $.post(file,{ 'process':'guardarMecMet', 'd':datos},function(r){
        ...
        $("#ResultadoModal").fadeIn(500).html(r.Mensaje);
        if(r.Codigo==1){
            $( "#id_metodos" ).focus(); 
